...
namespace LabraryProject
{
	public partial class adminbookinventory : System.Web.UI.Page
	{
		...
		protected void Button1_Click(object sender, EventArgs e)
		{
			if (checkIfBoobExist())
			{
				Response.Write("<script>alert('Введите другой экземпляр для книги');</script>");
			}
			else
			{
				addNewBook();
			...
			if (checkIfBoobExist())
			{
				try
				{
					...
					else
					{
						...
						{
							Response.Write("<script>alert('Такого быть не может');</script>");
							return;
					...
					string filepath = "~/book_img/bookdefault";
		...
		void getBookById()
		{
			try
			{
				...
				if (dt.Rows.Count >= 1)
				{
					...
					TextBox7.Text = "" + (Convert.ToInt32(dt.Rows[0]["actual_stock"].
						ToString()) - Convert.ToInt32(dt.Rows[0]["current_stock"].ToString()));
				...
				{
					Response.Write("<script>alert('Данной книги не существует');</script>");
				}
		...
		void addNewBook()
		{
			try
			{
				...
				genres = genres.Remove(genres.Length - 1);
				// tesrt
				string filepath = "~/book_img/bookdefault.png";
				string filename = Path.GetFileName(FileUpload1.PostedFile.FileName);
				FileUpload1.SaveAs(Server.MapPath("book_img/" + filename));
				filepath = "~/book_img/" + filename;
		...
		void deleteBookByID()
		{
			...
			{
				Response.Write("<script>alert('Книги нет');</script>");
			}
		} */
		string strcon = ConfigurationManager.ConnectionStrings["con"].ConnectionString;
		static string global_filepath;
		...
		protected void Button1_Click(object sender, EventArgs e)
		{
			if (checkIfBookExists())
			{
				Response.Write("<script>alert('Данная книга существует');</script>");
			}
			else
			{
				addNewBook();
		...
		void deleteBookByID()
		{
			...
			{
				Response.Write("<script>alert('Неверное имя');</script>");
			}
		}
		...
		void getBookByID()
		{
			try
			{
				...
				{
					Response.Write("<script>alert('Неверное книги');</script>");
				}
