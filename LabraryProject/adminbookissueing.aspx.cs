	...
	public partial class adminbookissueing : System.Web.UI.Page
	{
		...
		protected void Button2_Click(object sender, EventArgs e)
		{
			if (checkIfBookExist() && checkIfMemberExist())
			{
				...
				{
					Response.Write("<script>alert('Уже взял');</script>");
				}
				else
				{
					issueBook();
			...
			{
				Response.Write("<script>alert('Неверное ID книги или пользователя');</script>");
			}
		}
		// return book
		protected void Button4_Click(object sender, EventArgs e)
		{
			if (checkIfBookExist() && checkIfMemberExist())
			{
				...
				{
					Response.Write("<script>alert('Нету данных');</script>");
				}
		...
		void returnBook()
		{
			try
			{
				...
				if (result > 0)
				{
					...
					Response.Write("<script>alert('Книга возвращена');</script>");
					GridView1.DataBind();
				...
				{
					Response.Write("<script>alert('Неверные детали');</script>");
				}
		...
		void getNames()
		{
			try
			{
				...
				{
					Response.Write("<script>alert('Неверное Id книги');</script>");
				}
				...
				{
					Response.Write("<script>alert('Неверное Id пользователя');</script>");
				}
